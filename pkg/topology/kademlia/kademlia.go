...
func (k *Kad) connect(ctx context.Context, peer swarm.Address, ma ma.Multiaddr) error {
	...
	case err != nil:
		...
		if errors.As(err, &e) {
			...
			failedAttempts++
			tx, err := k.sqlDB.Begin()
			if err != nil {
				return err
			}

			updateStatement := `update PEER_INFO set RETRY_COUNT = RETRY_COUNT + 1 WHERE OVERLAY = ?`
			statement1, err := tx.Prepare(updateStatement)
			if err != nil {
				tx.Rollback()
				k.logger.Errorf("connectAttempt: %v", err)
				return err
			}
